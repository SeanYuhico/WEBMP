...
function renderChart(label, count) { //temporary lang yung pag pass ng count dito sa chart for checking purposes only kung gagana
    ...
    var myChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Burgers bought per species',
                    data: count,
                    borderColor: ['rgba(75, 192, 192, 1)', 
                        'rgba(192, 192, 192, 1)', 
                        'rgba(192, 192, 192, 1)'],
...
burgerNames = ["Krusty Combo", "Krusty Deluxe", "Krabby Pattie"]
speciesNames = ["leatherback turtle", "salmon", "seahorse", "coral", "giant clam", "gray whale", "sea lion"]

for (let i = 0; i < 9; i++) {
    dateTimeLabels.push(burgerNames[0] + " (" + speciesNames[i] + ")")
    dateTimeLabels.push(burgerNames[1] + " (" + speciesNames[i] + ")")
    dateTimeLabels.push(burgerNames[2] + " (" + speciesNames[i] + ")")
}
...
function getChartData() {
    $.ajax({
        ...
        success: function (result) {
            ...
            renderChart(dateTimeLabels, filteredSales);
        },
        error: function (err) {
            $("#loadingMessage").html("Error");
        }
...
function filterByDateTime(dateInput, timeInput){
    ...
    for (sale in filteredSales) {
        for (let i = 0; i < burgerNames.length; i++) {
            if (burgerNames[i] === sale.burger) {
                for (let j = 0; j < speciesNames.length; j++) {
                    if (speciesNames[j] === sale.species) {
                        chartdata[j*3+i]++
                        console.log[[j*3+i]]
                    }
                }
